# https://forums.raspberrypi.com/viewtopic.php?t=302978#p1823668 By danjperron
#       2022.09.14  V0.0    add read/write word data
...
class i2c_slave:
    ...
    def putWord(self, data):
        ...
        status = mem32[ self.i2c_base | self.IC_CLR_RD_REQ]
        mem32[ self.i2c_base | self.IC_DATA_CMD] = data  & 0xff
        mem32[ self.i2c_base | self.IC_DATA_CMD] = (data >> 8) & 0xff
    ...
    def getWord(self):
        while not self.any():
            pass
        work =  mem32[ self.i2c_base | self.IC_DATA_CMD] & 0xff
        while not self.any():
            pass
        return ((mem32[ self.i2c_base | self.IC_DATA_CMD] & 0xff) << 8) | work
